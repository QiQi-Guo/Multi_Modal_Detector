...
class SparseEncoder(nn.Module):
    ...
    def forward(self, voxel_features, coors, batch_size, **kwargs):
        ...
        try:
            x = self.conv_input(input_sp_tensor)   # CUDA error: an illegal memory access was encountered ???  -qixi
        except:
            # print(voxel_features)
            # print(coors)
            print('voxel_feature')
            print(torch.isnan(voxel_features).sum() > 0)
            print(torch.isinf(voxel_features).sum() > 0)
            print("coors")
            print(torch.isnan(coors).sum() > 0)
            print(torch.isinf(coors).sum() > 0)
