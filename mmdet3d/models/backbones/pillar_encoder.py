...
@BACKBONES.register_module()
class PointPillarsScatter_with_conv(nn.Module):    # qixi designed model
    def __init__(self, in_channels=64, output_shape=(512, 512), **kwargs):
        """
        Point Pillar's Scatter.
        Converts learned features from dense tensor to sparse pseudo image. This replaces SECOND's
    ...
    def forward(self, voxel_features, coords, batch_size):
        ...
        batch_canvas = self.relu(self.bn_1(self.conv_1(batch_canvas)))
        short_cut = batch_canvas
        batch_canvas = self.relu(self.bn_2(self.conv_2(batch_canvas)))
        batch_canvas = self.relu(self.bn_3(self.conv_3(batch_canvas)))
        batch_canvas = batch_canvas + short_cut
        short_cut = batch_canvas
        batch_canvas = self.relu(self.bn_4(self.conv_4(batch_canvas)))
        batch_canvas = self.relu(self.bn_5(self.conv_5(batch_canvas)))
        batch_canvas = batch_canvas + short_cut
        batch_canvas = self.relu(self.bn_final(self.conv_final(batch_canvas)))
        return batch_canvas
