...
class fast_BEV(nn.Module):
    def __init__(self,
        ...
        self.trans_bias = 0.1  # m
        self.rot_bias = 0.5  # deg
        self.T_matrix_bias = False

        self.visualize_times = 0
    ...
    def forward(self,
        ...
        for i in range(mlvl_feats.shape[0]):
            ...
            # projection = (img_aug_m_r @ lidar2image_m @ torch.inverse(lidar_aug_m))
            ...
            volume = torch.zeros((n_channels, point.shape[-1]), device=feat_i.device).type_as(
            ...
            if self.visualize_count < self.visualize_times:
                ...
                ori_point_i = torch.cat((ori_point_i, torch.ones_like(ori_point_i[:, :1])), dim=1).expand(n_images, 4,
                                                                                                          -1)
        ...
        if mask != None:
            volume_list = torch.stack(volumes) * mask.permute(0, 2, 3, 1).unsqueeze(1)  # list([bs, c, vx, vy, vz])
        else:
